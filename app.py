...
class BankCustomerSegmentationDashboard:
    ...
    def display_kpi_metrics(self, filtered_df):
        ...
        metrics_row1 = [
            {'title': 'Total Transactions', 'value': f"{total_transactions:,}", 'color': '#1f77b4', 'icon': '🔄'},
            {'title': 'Unique Customers', 'value': f"{unique_customers:,}", 'color': '#9467bd', 'icon': '👥'},
            {'title': 'Total Amount', 'value': f"₹{total_amount:,.0f}", 'color': '#2ca02c', 'icon': '💰'},
            {'title': 'Avg Transaction', 'value': f"₹{avg_transaction:,.0f}", 'color': '#ff7f0e', 'icon': '📈'},
        ]
        
        metrics_row2 = [
            {'title': 'Avg Balance', 'value': f"₹{avg_balance:,.0f}", 'color': '#d62728', 'icon': '🏦'},
            {'title': 'Max Transaction', 'value': f"₹{max_transaction:,.0f}", 'color': '#8c564b', 'icon': '⚡'},
            {'title': 'Min Balance', 'value': f"₹{min_balance:,.0f}", 'color': '#e377c2', 'icon': '📉'},
            {'title': 'Max Balance', 'value': f"₹{max_balance:,.0f}", 'color': '#7f7f7f', 'icon': '🚀'},
        ]
    ...
    def display_data_insights(self, filtered_df):
        ...
        with col1:
            ...
            insights_data = {
                ...
                'Value': [
                    f"₹{filtered_df['TransactionAmount (INR)'].sum():,.0f}",
    ...
    def run(self):
        ...
        with col2:
            ...
            <div style='text-align: center; color: #888;'>
                ...
                <p style='margin: 5px 0;'>📊 Real-time Analytics • 🗺️ Geospatial Insights • 🎯 Customer Segmentation</p>
            </div>
            """, unsafe_allow_html=True)
